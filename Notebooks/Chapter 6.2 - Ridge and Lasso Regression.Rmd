---
title: "Chapter 6.2 - Ridge and Lasso Regression"
...
This notebook corresponds with Chapter 6 of Inroduction to Statistical Learning
...
Before continuing, take a look at the glmnet() vignette to get an idea of what we need beforehand
  ...
  - lambda - a user created decreasing sequence of lambda values. Lambda will be a vector of length 100, ranging from 10^10 to 10^-2
...
here are two common ways to randomly split a data set:

  1. You can produce a random logical vector (TRUE/FALSE) and select observations corresponding to TRUE for the training data
  2. Alternatively, randomlychoose a subset of numbers between 1 and n, which can then be used as the indices for the training data
...
ridge_pred <- predict(ridge_mod, s=4, newx=x[test,]) #s option sets the lambda value, newx specifies new observations used to make predictions
...
```{R}
ridge_pred <- predict(ridge_mod, x= x[train,], y = y[train], newx=x[test,], s=1e10, exact = TRUE)
...
```{r}
ridge_pred <- predict(ridge_mod, x= x[train,], y = y[train], newx=x[test,], s=0, exact = TRUE)
 #the exact option allow us to specify that lambda is exactly 0, instead of searching for the smallest value of lambda in "grid"
mean((ridge_pred - y.test)^2)
```
